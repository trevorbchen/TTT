...
def main(config: DictConfig):
    ...
    for i in pbar:
                ...
                # --- Jacobian matching loss (2nd-order, create_graph=True) ---
                # Trains cos_sim(∇_denoised ||f_θ-y||², ∇_denoised ||A-y||²) → 1
                # Requires backprop through the surrogate gradient (mixed 2nd-order derivative).
                ...
                if jac_loss and not is_grad_pred and target_mode == "forward" and denoised is not None:
                    # True operator gradient (detached — fixed target)
                    ...
                    pred_surr = classifier(x_noisy, sigma, None, denoised=denoised_jac2)
